...
func (e *Endpoint) Update(owner Owner, opts models.ConfigurationMap) error {
	scopedLog := log.WithFields(logrus.Fields{logfields.EndpointID: e.ID, "configuration-options": opts})
	scopedLog.Debug("updating configuration options for endpoint")
	...
	if needToRegenerate {
		scopedLog.Debug("need to regenerate endpoint; checking state before" +
		...
		for {
			...
			case <-timeout:
				scopedLog.Warningf("timed out waiting for endpoint state to change")
				e.Mutex.Unlock()
				ctCleaned.Wait()
