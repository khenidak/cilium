...
type CiliumNetworkPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v3.CiliumNetworkPolicy, err error)
...
// List lists all CiliumNetworkPolicies in the indexer.
func (s *ciliumNetworkPolicyLister) List(selector labels.Selector) (ret []*v3.CiliumNetworkPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v3.CiliumNetworkPolicy))
	})
	return ret, err
}
...
type CiliumNetworkPolicyNamespaceLister interface {
	...
	Get(name string) (*v3.CiliumNetworkPolicy, error)
...
// List lists all CiliumNetworkPolicies in the indexer for a given namespace.
func (s ciliumNetworkPolicyNamespaceLister) List(selector labels.Selector) (ret []*v3.CiliumNetworkPolicy, err error) {
...
// Get retrieves the CiliumNetworkPolicy from the indexer for a given namespace and name.
func (s ciliumNetworkPolicyNamespaceLister) Get(name string) (*v3.CiliumNetworkPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v3.Resource("ciliumnetworkpolicy"), name)
	}
	return obj.(*v3.CiliumNetworkPolicy), nil
}
